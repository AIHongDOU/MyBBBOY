...
// 用户无需手动输入即可直接开始对话。
const DEFAULT_DIRECT_URL = "ws://localhost:8765/v1/realtime";
...
function loadSettings() {
  return {
    // 未保存过地址时使用默认本地地址，避免“语音服务地址”为空导致无法开始对话
...
/** 底部实时字幕条元素（数字人界面） */
const subtitleBar = /** @type {HTMLElement} */ ($("#subtitle-bar"));
let subtitleTimer = 0;

/** 更新底部实时字幕条：显示最近一句文字，短暂停留后淡出。
 *  @param {string} text 要显示的字幕文本 */
function updateSubtitle(text) {
  if (!text) return;
  // 聊天字幕中不出现“助手”二字，统一移除（角色定义为“小姚”）
  subtitleBar.textContent = text.replace(/助手/g, "");
...
function syncConnectionUi() {
  ...
  if (pinnedUrl) {
    ...
    connHint.textContent = "语音服务地址已由当前部署固定。";
  } else if (allowDirect) {
    // 直连模式：由用户自行填写语音服务地址
    connField.hidden = false;
    ...
    connHint.textContent =
      "你的语音服务地址，例如 http://localhost:8080（应用会自动加上 /v1/realtime）。";
  } else {
...
function promptServerUrl() {
  ...
  connHint.textContent = "请先设置语音服务地址再开始。";
