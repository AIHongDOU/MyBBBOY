...
export class ChatView {
  ...
  _buildMessageEl({ container, prefix, role, text, partial = false }) {
    ...
    // 角色名已定义为“小姚”：用户侧显示“我”，数字人侧显示“小姚”，不再使用“You/Assistant/助手”
    const label = role === "user" ? "我" : "小姚";
    // 聊天字幕中不出现“助手”二字，统一移除
    const clean = (text || "").replace(/助手/g, "");
    el.innerHTML = `<div class="${prefix}-role">${label}</div><div class="${prefix}-body${partial ? " partial" : ""}"${clean ? "" : " hidden"}>${escHtml(clean)}</div>`;
    return el;
  }
